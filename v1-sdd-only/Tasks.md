...
- [x] T29 实现复习概览接口：`GET /api/schedule/overview` 和 `GET /api/schedule/calendar`（打卡日历）
- [x] T30 实现复习算法：ReviewScheduleService.updateStage() — 认识(stage+1，最高7)、模糊(stage-1，不低于0)、不认识(stage重置为0)
- [x] T31 实现复习算法：ReviewScheduleService.calcNextReview() — 根据 stage 查间隔表，计算 `next_review_time = now + intervalMinutes[stage]`，stage=7 且认识时 status 设为 1（已掌握）
- [x] T32 实现复习提交：复用 `POST /api/study/submit` — 更新 review_plan，写入 review_log 记录
- [x] T33 前端：Schedule.vue — 展示今日待复习词，用户提交复习结果

## 阶段六：错词 + 打卡 + 统计

- [x] T34 设计 wrong_word 表 + checkin_record 表，建表 SQL
- [x] T35 实现错词归集：学习 result=不认识，或复习 result=不认识/模糊 → WrongWordService 写入或更新 wrong_word 表
- [x] T36 实现错词查询接口：`GET /api/wrong-words/page`（分页）、`GET /api/wrong-words/reinforce`（待强化）、`GET /api/wrong-words/export`（导出）、`POST /api/wrong-words/{id}/mastered`（标记掌握）、`DELETE /api/wrong-words/{id}`（删除）
- [x] T37 实现每日签到：CheckinService — 用户在打卡页面手动签到
- [x] T38 实现签到接口：`POST /api/schedule/checkin`
- [x] T39 实现统计查询：StatisticsService — 学习总览（天数、掌握数、各轮次分布）、每日趋势（折线图数据）
- [x] T40 实现统计接口：`GET /api/statistics/overview`、`GET /api/statistics/trend`、`GET /api/statistics/weak-analysis`（薄弱词分析）
- [x] T41 前端：Statistics.vue — 统计可视化
- [x] T42 前端：WrongWords.vue — 错词列表，支持重新学习
...
- [x] T46 实现公告管理接口（`/api/admin/announcements` — CRUD）+ 用户端公告获取接口（`GET /api/announcements`）
- [x] T47 实现操作日志：OperationLogInterceptor — 自动记录管理员的每次操作
- [x] T48 实现操作日志查询接口：`GET /api/admin/operation-logs`
- [x] T49 前端：后台管理页面（AdminDashboard）— 用户管理 + 公告管理 + 操作日志
...
- [x] T53 测试用例：正确登录成功 / 错误密码返回错误提示 / token 过期返回 401
- [x] T54 测试用例：复习结果提交后 review_plan.stage 正确更新 / review_log 正确写入
