...
CREATE TABLE IF NOT EXISTS `wrong_word` (
    ...
    `word_id`     BIGINT  NOT NULL COMMENT '错误单词 ID',
    `wrong_count` INT     NOT NULL DEFAULT 1 COMMENT '累计错误次数',
    `status`      INT     NOT NULL DEFAULT 0 COMMENT '状态：0=待复习 1=已掌握',
    `created_at`  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '首次错误时间',
...
CREATE TABLE IF NOT EXISTS `checkin_record` (
    ...
    `completed_target` INT     NOT NULL DEFAULT 0 COMMENT '是否完成当日目标：0=未完成 1=已完成',
    PRIMARY KEY (`id`),
